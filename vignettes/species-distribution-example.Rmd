---
title: "End-to-End Species Distribution Modeling with maxentcpp"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{End-to-End Species Distribution Modeling with maxentcpp}
...
This vignette demonstrates a complete species distribution modeling (SDM)
workflow using **maxentcpp**, from loading environmental raster data and
species occurrence records through model training, evaluation, projection,
and diagnostics.
...
`maxentcpp` works with its own internal grid format. The package provides
bridge functions for the **terra** and **raster** packages so you can
load `.tif` (GeoTIFF) or any format those packages support.
...
`maxent_read_occurrences()` accepts a CSV file path or an R data frame
(e.g. from `rgbif::occ_data()`) and converts geographic coordinates to
grid row/column indices.
...
For users who want a complete Java-Maxent-style output directory in a single
call, `maxent_run()` orchestrates the full pipeline — from occurrence records
to an HTML report — and writes all the standard output files.
