---
title: "Virtual Species Validation: maxentcpp vs maxnet"
---
...
has_maxnet <- requireNamespace("maxnet", quietly = TRUE)
has_terra  <- requireNamespace("terra", quietly = TRUE)
can_run    <- has_maxnet && has_terra
```
...
```{r load-env, eval = can_run}
library(maxentcpp)
library(terra)

stack_path <- system.file("extdata", "stack_1_12_crop.rds",
                          package = "maxentcpp")
env <- terra::rast(readRDS(stack_path))
bio1  <- env[[1]]
bio12 <- env[[2]]
...
```{r virtual-species, eval = can_run}
...
```{r sample-presences, eval = can_run}
set.seed(42)
...
```{r fit-maxnet, message=FALSE, eval = can_run}
library(maxnet)
...
```{r comparison-maps, fig.width=10, fig.height=7, fig.alt="Side-by-side maps showing true suitability, maxentcpp prediction, maxnet prediction, and the difference between maxentcpp and maxnet.", eval = can_run}
par(mfrow = c(2, 2))
plot(true_rast,
...
```{r response-curves, fig.width=10, fig.height=5, fig.alt="Response curves showing predicted suitability as a function of bio1 and bio12 for the virtual species.", eval = can_run}
par(mfrow = c(1, 2))
...
4. `maxentcpp` faithfully reproduces the original Maxent optimizer while
   operating entirely in memory without Java dependencies.
