...
GitHub Actions already runs `R CMD check` for pushes and pull requests via
[.github/workflows/test-r-package.yml](.github/workflows/test-r-package.yml),
which exercises the `testthat` suite on Ubuntu, Windows, and macOS.

## Quick Start

The package ships with a complete reproducible example in
[`inst/examples/quickstart.R`](inst/examples/quickstart.R).
...
- [x] Complete end-to-end workflow examples (`inst/examples/quickstart.R`)
- [x] Canonical color ramp (`maxent_color_ramp()`) matching Java Maxent
- [x] Prediction map PNG writer (`maxent_write_prediction_png()`)
- [x] Response curve PNG writer (`maxent_plot_response_curves()`)
- [x] Variable importance bar chart (`maxent_plot_variable_importance()`)
- [x] Omission/threshold CSV (`maxent_write_omission_csv()`)
- [x] Sample predictions CSV (`maxent_write_sample_predictions()`)
- [x] Console results printer replicating dismo output (`maxent_print_results()`)
- [x] `maxentResults.csv` appender (`maxent_append_results_csv()`)
- [x] High-level one-click wrapper (`maxent_run()`)
- [ ] CRAN release
